...
export function KelolaMateriClient({
  initialMaterials,
  subjects,
  classes,
  ...
  const [activeTab, setActiveTab] = useState<'materi' | 'izin-guru'>('materi')
  const [search, setSearch] = useState('')
  const [teacherSearch, setTeacherSearch] = useState('')
  const [statusFilter, setStatusFilter] = useState<'ALL' | 'BELUM_DITUGASKAN' | 'PROGRESS' | 'SELESAI'>('ALL')
  ...
  const filteredMaterials = materials.filter((m: any) => {
    const matchesSearch = 
      ...
      (m.class?.name || 'Semua Kelas').toLowerCase().includes(search.toLowerCase()) ||
  ...
  const handleBulkStatusUpdate = async (status: string) => {
    ...
    const toastId = toast.loading(`Mengubah status ${selectedIds.length} materi...`)
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success(`Status ${selectedIds.length} materi berhasil diperbarui`, { id: toastId })
        setSelectedIds([])
      } else {
  ...
  const handleBulkDelete = async () => {
    if (selectedIds.length === 0) return
    if (!confirm(`Apakah Anda yakin ingin menghapus ${selectedIds.length} materi terpilih?`)) return
    
    setIsSubmitting(true)
    const toastId = toast.loading(`Menghapus ${selectedIds.length} materi...`)
    ...
    try {
      ...
      if (res.ok) {
        setMaterials(materials.filter((m: any) => !selectedIds.includes(m.id)))
        toast.success(`${selectedIds.length} materi berhasil dihapus`, { id: toastId })
        setSelectedIds([])
      } else {
        const err = await res.text()
        toast.error(err || 'Gagal menghapus materi secara massal', { id: toastId })
      }
    } catch (error) {
      toast.error('Terjadi kesalahan jaringan', { id: toastId })
    } finally {
  ...
  const openAdd = () => {
    if (!canEditMaterials) {
      toast.error('Akses ditolak: Anda tidak memiliki izin untuk mengedit materi')
      return
    }
    setFormData({
      title: '',
  ...
  const openEdit = (material: any) => {
    ...
    if (material.attachments) {
      try {
        parsedAttachments = typeof material.attachments === 'string' 
          ? JSON.parse(material.attachments) 
  ...
  const openDelete = (material: any) => {
    if (!canEditMaterials) {
      toast.error('Akses ditolak: Anda tidak memiliki izin untuk menghapus materi')
      return
    }
    setSelectedMaterial(material)
  ...
  const handleAddMaterial = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.text()
        toast.error(err || 'Gagal mengunggah materi', { id: toastId })
      }
    } catch (error) {
      toast.error('Terjadi kesalahan jaringan', { id: toastId })
    } finally {
  ...
  const handleEditMaterial = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMaterials(materials.map((m: any) => m.id === selectedMaterial.id ? updatedMat : m))
  ...
  const handleDeleteMaterial = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setMaterials(materials.filter((m: any) => m.id !== selectedMaterial.id))
        toast.success('Materi berhasil dihapus', { id: toastId })
        setIsDeleteOpen(false)
      } else {
        const err = await res.text()
        toast.error(err || 'Gagal menghapus materi', { id: toastId })
      }
    } catch (error) {
      toast.error('Terjadi kesalahan jaringan', { id: toastId })
    } finally {
  ...
  const handlePermissionToggle = async (teacherId: string, currentStatus: boolean) => {
    ...
    try {
      const res = await fetch(`/api/admin/users/${teacherId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ canEditMaterials: nextStatus })
      })

      if (res.ok) {
        setTeachers(teachers.map((t: any) => 
          t.id === teacherId ? { ...t, canEditMaterials: nextStatus } : t
        ))
        toast.success('Izin edit materi berhasil diperbarui', { id: toastId })
      } else {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'PROGRESS':
        return (
          <Badge className="bg-blue-500/10 text-blue-500 border border-blue-500/20 px-2.5 py-1 rounded-md text-[11px] font-bold flex items-center gap-1.5 w-fit">
      ...
      default:
        return (
          <Badge className="bg-amber-500/10 text-amber-500 border border-amber-500/20 px-2.5 py-1 rounded-md text-[11px] font-bold flex items-center gap-1.5 w-fit">
            <AlertCircle className="h-3.5 w-3.5" /> Belum Ditugaskan
          </Badge>
        )
    }
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-sm text-[var(--muted-foreground)] font-medium mt-2 flex items-center gap-3">
            Manajemen draf, penugasan, status, dan lampiran materi mata pelajaran.
      ...
      {!canEditMaterials && role === 'TEACHER' && (
        ...
        >
          ...
          <div className="space-y-1">
            <h4 className="text-sm font-bold text-[var(--foreground)]">Akses Penyuntingan Terkunci</h4>
            <p className="text-xs text-[var(--muted-foreground)] leading-relaxed">
              Anda tidak memiliki izin edit dari Administrator. Anda hanya dapat melihat daftar materi milik Anda sendiri. Silakan hubungi admin sekolah untuk mengaktifkan izin "Bisa Edit Materi".
            </p>
          </div>
        </motion.div>
      )}
      ...
      {role === 'ADMIN' && (
        <div className="flex border-b border-[var(--border)] gap-6">
          ...
          >
            ...
            {activeTab === 'materi' && (
              <motion.div
                layoutId="active-materi-tab-indicator"
          ...
          >
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
              ...
              <div className="flex bg-[var(--muted)]/50 p-1 rounded-xl border border-[var(--border)] w-full md:w-auto overflow-x-auto scrollbar-hide">
                {(['ALL', 'BELUM_DITUGASKAN', 'PROGRESS', 'SELESAI'] as const).map((status) => (
                  <button
                    key={status}
                    onClick={() => {
                  ...
                  >
                    {status === 'ALL' ? 'Semua' : status === 'BELUM_DITUGASKAN' ? 'Belum Ditugaskan' : status === 'PROGRESS' ? 'Progress' : 'Selesai'}
                  </button>
                ))}
              </div>
              ...
              <div className="relative group w-full md:w-80">
                ...
                <Input
                  placeholder="Cari materi, mapel, kelas..."
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="border-[var(--border)] hover:bg-transparent bg-[var(--muted)]/30">
                        ...
                        <TableHead className="text-xs font-bold text-[var(--muted-foreground)]">Judul Materi</TableHead>
                        ...
                        {role === 'ADMIN' && (
                          <TableHead className="text-xs font-bold text-[var(--muted-foreground)]">Pengampu</TableHead>
                        )}
                    ...
                    <TableBody>
                      {filteredMaterials.length === 0 ? (
                        <TableRow>
                          <TableCell
                            colSpan={canEditMaterials ? (role === 'ADMIN' ? 8 : 7) : (role === 'ADMIN' ? 7 : 6)}
                            className="h-64 text-center"
                          >
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                              <BookOpen className="h-12 w-12 text-[var(--muted-foreground)] opacity-50 mx-auto mb-4" />
                              <p className="text-sm font-bold text-[var(--muted-foreground)]">Tidak ada materi ditemukan</p>
                            </motion.div>
                          </TableCell>
                        </TableRow>
                      ) : (
                        filteredMaterials.map((mat: any, index: number) => {
                          let attachmentsList = []
                          if (mat.attachments) {
                            try {
                              attachmentsList = typeof mat.attachments === 'string'
                          ...
                          return (
                            <motion.tr
                              ...
                              className={`border-[var(--border)] hover:bg-[var(--muted)]/30 transition-colors group ${
                                selectedIds.includes(mat.id) ? 'bg-[#5483B3]/5' : ''
                              }`}
                            >
                              {canEditMaterials && (
                                <TableCell className="pl-4 md:pl-6">
                                  <button
                                    ...
                                    className={`transition-colors ${
                                      selectedIds.includes(mat.id) ? 'text-[#5483B3]' : 'text-[var(--muted-foreground)] hover:text-[var(--foreground)]'
                                    }`}
                                  >
                                    {selectedIds.includes(mat.id) ? <CheckSquare className="h-4.5 w-4.5" /> : <Square className="h-4.5 w-4.5" />}
                                  </button>
                                </TableCell>
                              )}
                              <TableCell className="max-w-xs md:max-w-sm">
                                <div className="py-2.5">
                                  <h4 className="text-sm font-bold text-[var(--foreground)] leading-snug">{mat.title}</h4>
                                ...
                                <TableCell>
                                  <div className="flex items-center gap-2">
                                    <Avatar className="h-7 w-7 rounded-full border border-[var(--border)]">
                                      <AvatarImage src={mat.teacher?.image} />
              ...
              {selectedIds.length > 0 && (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-xs font-bold text-[var(--foreground)]">Materi Terpilih</p>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 w-full md:w-auto md:ml-auto justify-center">
                    <Button
                      ...
                      onClick={() => handleBulkStatusUpdate('BELUM_DITUGASKAN')}
                    >
                      <AlertCircle className="h-3.5 w-3.5 text-amber-500" /> Draf
                    </Button>
                    <Button
                      variant="outline"
                      disabled={isSubmitting}
                    ...
                    >
                      <Clock className="h-3.5 w-3.5 text-blue-500" /> Progress
                    </Button>
                    <Button
                      variant="outline"
                      disabled={isSubmitting}
                    ...
                    >
                      <CheckCircle2 className="h-3.5 w-3.5 text-emerald-500" /> Selesai
                    </Button>
                    <Button
                      variant="destructive"
                      disabled={isSubmitting}
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-[var(--border)] hover:bg-transparent bg-[var(--muted)]/30">
                      ...
                      <TableHead className="text-xs font-bold text-[var(--muted-foreground)] text-right pr-6">Izin Edit Materi</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                        ...
                        <TableRow key={teach.id} className="border-[var(--border)] hover:bg-[var(--muted)]/20 transition-colors">
                          ...
                          <TableCell className="text-right pr-6">
                            <div className="flex items-center justify-end gap-3">
                              <span className={`text-xs font-semibold ${teach.canEditMaterials ? 'text-[#5483B3]' : 'text-[var(--muted-foreground)]'}`}>
                                {teach.canEditMaterials ? 'Diizinkan' : 'Terkunci'}
                              </span>
                              <Switch
                                checked={teach.canEditMaterials}
                                onCheckedChange={() => handlePermissionToggle(teach.id, teach.canEditMaterials)}
                              />
                            </div>
                          </TableCell>
                        </TableRow>
        ...
        <DialogContent className="bg-[var(--card)] border-[var(--border)] text-[var(--foreground)] max-w-2xl rounded-2xl shadow-xl p-0 overflow-hidden max-h-[90vh]">
          <div className="p-6 md:p-8 overflow-y-auto max-h-[85vh] custom-scrollbar">
            <DialogHeader className="mb-6">
              <DialogTitle className="text-2xl font-extrabold tracking-tight">Unggah Materi Pembelajaran</DialogTitle>
              <DialogDescription className="text-sm font-medium mt-1">Lengkapi data untuk mengunggah materi baru ke kelas.</DialogDescription>
            </DialogHeader>

            <form onSubmit={handleAddMaterial} className="space-y-6">
              <div className="space-y-4">
                <div className="space-y-2">
                  <Label className="text-xs font-bold text-[var(--foreground)]">Judul Materi</Label>
                  <Input
                    required
                ...
                <div className="space-y-2">
                  <Label className="text-xs font-bold text-[var(--foreground)]">Deskripsi / Ringkasan</Label>
                  <Textarea
                    value={formData.description}
                    onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                    className="bg-[var(--card)] border-[var(--border)] rounded-xl min-h-[100px] text-sm focus-visible:ring-[#5483B3] p-3"
                    placeholder="Tulis ringkasan singkat isi materi..."
                  />
                </div>
                ...
                <div className="space-y-2">
                  <Label className="text-xs font-bold text-[var(--foreground)]">Status Penugasan Awal</Label>
                  <Select
                  ...
                  >
                    ...
                    <SelectContent className="bg-[var(--card)] border-[var(--border)] rounded-xl shadow-lg z-50">
                      <SelectItem value="BELUM_DITUGASKAN">Draft / Belum Ditugaskan</SelectItem>
                      <SelectItem value="PROGRESS">Dalam Proses / Progress</SelectItem>
                ...
                <div className="space-y-3 pt-2">
                  <Label className="text-xs font-bold text-[var(--foreground)] flex items-center gap-1.5">
                    <LinkIcon className="h-4 w-4 text-[#5483B3]" /> Lampiran / Tautan Materi
                  </Label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-5 gap-2.5 items-end bg-[var(--muted)]/20 p-3.5 border border-[var(--border)] border-dashed rounded-xl">
                    <div className="sm:col-span-2 space-y-1.5">
                      ...
                      <Input
                        ...
                        placeholder="e.g. Modul PDF, Video YouTube"
              ...
              <div className="flex gap-4 pt-4 border-t border-[var(--border)]">
                ...
                >
                  {isSubmitting ? 'Memproses...' : 'Unggah Materi'}
                </Button>
              </div>
            </form>
          </div>
      ...
      {/* 2. Edit Material Dialog */}
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="bg-[var(--card)] border-[var(--border)] text-[var(--foreground)] max-w-2xl rounded-2xl shadow-xl p-0 overflow-hidden max-h-[90vh]">
          <div className="p-6 md:p-8 overflow-y-auto max-h-[85vh] custom-scrollbar">
            <DialogHeader className="mb-6">
              <DialogTitle className="text-2xl font-extrabold tracking-tight">Edit Materi Pembelajaran</DialogTitle>
              <DialogDescription className="text-sm font-medium mt-1">Perbarui data atau lampiran untuk materi ini.</DialogDescription>
            </DialogHeader>
      ...
      <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
        <DialogContent className="bg-[var(--card)] border-[var(--border)] text-[var(--foreground)] rounded-2xl max-w-md p-0 overflow-hidden shadow-xl">
          <div className="p-8 text-center">
            ...
            <DialogTitle className="text-xl font-bold mb-2 text-center">Hapus Materi</DialogTitle>
            <p className="text-[var(--muted-foreground)] text-sm mb-6 leading-relaxed">
              Apakah Anda yakin ingin menghapus materi <span className="text-[var(--foreground)] font-bold">{selectedMaterial?.title}</span>?
